...
export function reduceCanvasTextEdit(
  ...
  switch (action.type) {
    ...
    case "textarea_dom_sync": {
      // Mirror the textarea DOM into the session. IME composition edits the
      // textarea directly (its beforeinput is non-cancelable), so the regular
      // intent pipeline is bypassed and this sync keeps the controlled value
      // in step — otherwise React restores the stale value and cancels the
      // composition.
      const session = state.session;
      if (!session) {
        return { state, effects: [] };
      ...
      const nextCompositionRange = action.composing ? { start: selection.start, end: selection.end } : null;
      if (session.text === action.text) {
        if (
          session.selectionStart === selection.start &&
          session.selectionEnd === selection.end &&
          (state.compositionRange == null) === (nextCompositionRange == null)
        ) {
          return { state, effects: [] };
        }
        return {
      ...
      const checkpointState = state.compositionRange == null ? withUndoCheckpoint(state, session) : state;
      const reduced = applySessionTextUpdate(checkpointState, action.text, selection.start, selection.end);
      return {
        state: {
          ...reduced.state,
          compositionRange: nextCompositionRange
