...
async function initializeWorkerRuntimeOnce(): Promise<MathJaxRuntime> {
  ...
  const runtime: MathJaxRuntime = {
    tex2svg,
    // handleRetriesFor awaits MathJax retry errors (dynamic font subset loads)
    // instead of surfacing them; without it the first render of text needing a
    // dynamic font fails, flushPending() reports no changes, and the node is
    // stuck as plain text until an unrelated re-layout.
    tex2svgPromise: (input, options) => mathjax.handleRetriesFor(() => tex2svg(input, options)),
    outputJax: svg,
    startup: {
      output: svg,
      adaptor: {
...
function splitExplicitMultilineSource(text: string): string[] {
  const lines = splitExplicitMultilineSegments(text).lines;
  // Balance TeX group braces per line: `\\` inside an open group (e.g.
  // `{\footnotesize a\\b}`) is a legal line break, but the split fragments are
  // measured individually and unbalanced braces make MathJax throw
  // "Missing close brace" / "Extra close brace" for perfectly valid input.
  ...
  for (const line of lines) {
    ...
    const open = Math.max(depth, -minNet);
    const close = Math.max(0, open + net);
    balanced.push("{".repeat(open) + line + "}".repeat(close));
...
/**
 * Remove unescaped `%` line comments the way TeX does: the comment runs to the
 * end of the line and also consumes the newline plus the next line's leading
 * whitespace. This must happen before newlines are collapsed to spaces below,
 * otherwise a trailing `{%` comment swallows the rest of the node text.
 */
function stripTexLineComments(text: string): string {
  if (!text.includes("%")) {
    return text;
  }
  let stripped = "";
  ...
  while (index < text.length) {
    const char = text[index];
    if (char === "\\") {
      stripped += text.slice(index, Math.min(text.length, index + 2));
      index += 2;
      continue;
    }
    if (char === "%") {
...
function normalizeMathJaxTextInput(
  ...
  let resolvedText = stripTexLineComments(text);
